...
from kesslergame import (
    KesslerController,
)  # In Eclipse, the name of the library is kesslergame, not src.kesslergame
...
class ScottDickController(KesslerController):
    def __init__(self):
        ...
        )  # Radians due to Python
        ship_turn = ctrl.Consequent(
            np.arange(-180, 180, 1), "ship_turn"
        )  # Degrees due to Kessler
        ...
        theta_delta["NL"] = fuzz.zmf(
            theta_delta.universe, -1 * math.pi / 3, -1 * math.pi / 6
        )
        theta_delta["NS"] = fuzz.trimf(
            theta_delta.universe, [-1 * math.pi / 3, -1 * math.pi / 6, 0]
        )
        theta_delta["Z"] = fuzz.trimf(
            theta_delta.universe, [-1 * math.pi / 6, 0, math.pi / 6]
        )
        theta_delta["PS"] = fuzz.trimf(
            theta_delta.universe, [0, math.pi / 6, math.pi / 3]
        )
        ...
        rule1 = ctrl.Rule(
            bullet_time["L"] & theta_delta["NL"], (ship_turn["NL"], ship_fire["N"])
        )
        rule2 = ctrl.Rule(
            bullet_time["L"] & theta_delta["NS"], (ship_turn["NS"], ship_fire["Y"])
        )
        rule3 = ctrl.Rule(
            bullet_time["L"] & theta_delta["Z"], (ship_turn["Z"], ship_fire["Y"])
        )
        rule4 = ctrl.Rule(
            bullet_time["L"] & theta_delta["PS"], (ship_turn["PS"], ship_fire["Y"])
        )
        rule5 = ctrl.Rule(
            bullet_time["L"] & theta_delta["PL"], (ship_turn["PL"], ship_fire["N"])
        )
        rule6 = ctrl.Rule(
            bullet_time["M"] & theta_delta["NL"], (ship_turn["NL"], ship_fire["N"])
        )
        rule7 = ctrl.Rule(
            bullet_time["M"] & theta_delta["NS"], (ship_turn["NS"], ship_fire["Y"])
        )
        rule8 = ctrl.Rule(
            bullet_time["M"] & theta_delta["Z"], (ship_turn["Z"], ship_fire["Y"])
        )
        rule9 = ctrl.Rule(
            bullet_time["M"] & theta_delta["PS"], (ship_turn["PS"], ship_fire["Y"])
        )
        rule10 = ctrl.Rule(
            bullet_time["M"] & theta_delta["PL"], (ship_turn["PL"], ship_fire["N"])
        )
        rule11 = ctrl.Rule(
            bullet_time["S"] & theta_delta["NL"], (ship_turn["NL"], ship_fire["Y"])
        )
        rule12 = ctrl.Rule(
            bullet_time["S"] & theta_delta["NS"], (ship_turn["NS"], ship_fire["Y"])
        )
        rule13 = ctrl.Rule(
            bullet_time["S"] & theta_delta["Z"], (ship_turn["Z"], ship_fire["Y"])
        )
        rule14 = ctrl.Rule(
            bullet_time["S"] & theta_delta["PS"], (ship_turn["PS"], ship_fire["Y"])
        )
        rule15 = ctrl.Rule(
            bullet_time["S"] & theta_delta["PL"], (ship_turn["PL"], ship_fire["Y"])
        )
    ...
    def actions(self, ship_state: Dict, game_state: Dict) -> Tuple[float, float, bool]:
        ...
        ship_pos_x = ship_state["position"][
            0
        ]  # See src/kesslergame/ship.py in the KesslerGame Github
        ...
        intrcpt_x = (
            closest_asteroid["aster"]["position"][0]
            + closest_asteroid["aster"]["velocity"][0] * bullet_t
        )
        intrcpt_y = (
            closest_asteroid["aster"]["position"][1]
            + closest_asteroid["aster"]["velocity"][1] * bullet_t
        )
